...
int userSelected = 0;
int numUsers = 0;
char* users[255]; //if we go over then seg fualt
int userIDs[255];

int DBConnect()
{
    con = mysql_init(NULL);
    if(mysql_real_connect(con, "192.168.50.69", "e30b67be", "25d258f3", "PiSmartScale", 3306, NULL, 0) == NULL)
        return -1;
...
double calcualteMass(int rawMassData, int calPos)
{
    ...
    if(rawMassData > calibration17[calPos])
        return 34 * ((rawMassData - calibration17[calPos]) / (calibration34[calPos] - calibration17[calPos]));
...
void calcuateWeight(double sample)
{
    ...
    if(sample > 25.00)
    {
        ...
        if(samples == sampleCount)
        {
            ...
            displayWrite("Connecting to", 0);
            displayWrite("Database...", 1);

            if(DBConnect() == 0)
            {
                clearDisplay();
                displayWrite("Save?", 0);
                displayWrite("Press Button!", 1);
                byte amemoryBuffer[23] = { 0 };
                int saveCountDown = 0;
                while(1)
                {
                    int bytes_read = read(dataPipe, amemoryBuffer, sizeof(amemoryBuffer));
                    if( bytes_read > 0 )
                    {
                        if(amemoryBuffer[1] == 0x32)
                        {
                            if(amemoryBuffer[3] == 0x0)
                            {
                                saveCountDown++;
                                if(saveCountDown == 1000)
                                    break;                      
                            }
                            if(amemoryBuffer[3] == 0x08)
                            {
                                printf("Log: Saving to Database...\n");
                                int usrID = userIDs[userSelected];
                                MYSQL_STMT* stmt = mysql_stmt_init(con);
                                std::string query = "INSERT INTO mass (id, date_time, measurement) values (?, CURRENT_TIMESTAMP(), ?)";
                                mysql_stmt_prepare(stmt, query.c_str(), query.length());
                                MYSQL_BIND bind[2];
                                memset(bind, 0, sizeof(bind));

                                bind[0].buffer_type = MYSQL_TYPE_LONG;
                                unsigned long v1_len = sizeof(usrID);
                                bind[0].buffer = &usrID;
                                bind[0].is_null = 0;
                                bind[0].length = &v1_len;

                                bind[1].buffer_type = MYSQL_TYPE_DOUBLE;
                                bind[1].buffer = &weight;
                                unsigned long v2_len = sizeof(weight);
                                bind[1].is_null= 0;
                                bind[1].length = &v2_len;
            ...
            {
                clearDisplay();
                displayWrite("No Database", 0);
                displayWrite("Connection :(", 1);
                sleep(2);
            }
        ...
        if( bytes_read > 0 )
        {
            ...
            if(memoryBuffer[1] == 0x32) //Board Event
            {
                ...
                if(memoryBuffer[3] == 0x08 && frontButtonPressed == 0) 
                {
                    ...
                    if(numUsers != 0)
                    {
                        printf("Log: User Change\n");
                        if(userSelected == numUsers - 1)
                        {
                            if(DBConnect() == 0)
                            {
                                loadUsers();
                        ...
                        clearDisplay();
                        displayWrite("User Selcted:", 0);
                        displayWrite(users[userSelected], 1);
                    }
                    else
                    {
                        if(DBConnect() == 0)
...
int main(int argc, char* argv[])
{
    ...
    {
        clearDisplay();
        displayWrite("Database Error", 0);
        displayWrite("No Users Loaded", 1);
        sleep(2);
    }
